...
def write_webapp_data(listings: Iterable[Listing], path: str, meta: dict) -> None:
    """Write the ``data.js`` the mobile dashboard reads.

    Embedded as a JS global (not fetched JSON) so the dashboard also works
    when opened directly from the filesystem on a phone.
    """
    payload = {"meta": meta, "listings": [item.to_dict() for item in listings]}
    body = json.dumps(payload, indent=2)
    with open(path, "w", encoding="utf-8") as fh:
        fh.write("/* Auto-generated by golf_scraper — do not edit by hand. */\n")
        fh.write("window.GOLF_DATA = ")
        fh.write(body)
        fh.write(";\n")
